...
class CSIM(UICC):
    ...
    def __init__(self):
        ...
        self.SELECT_ADF_CSIM()

    def SELECT_ADF_CSIM(self):
        # CSIM selection from AID
        if self.dbg:
            log(3, '(CSIM.__init__) UICC AID found:')
        self.get_AID()
        for aid in self.AID:
            print "Current AID: " 
            hexdump(str(bytearray(aid)))
            if  tuple(aid[0:5]) == (0xA0, 0x00, 0x00, 0x03, 0x43) \
            and tuple(aid[5:7]) == (0x10, 0x02) :
                csim = self.select(addr=aid, type='aid')
                if csim is None and self.dbg:
                    log(2, '(CSIM.__init__) CSIM AID selection failed')
                if csim is not None:
                    self.CSIM_AID = aid
                    if self.dbg:
                        log(3, '(CSIM.__init__) CSIM AID selection succeeded: ' + str(self.CSIM_AID) + '\n')
    
    def get_imsi_m(self):
        '''
        get_imsi_m() -> string(IMSI)
        
        reads IMSI_M value at address [0x6F, 0x22]
        returns IMSI_M string on success or None on error
        '''
        # select IMSI file
        imsi = self.select([0x6F, 0x22])
        if imsi is None: 
            return None
        ...
        if self.dbg >= 2: 
            log(3, '(get_imsi_m) %s' % self.coms())
        return None    

    def get_imsi_t(self):
        '''
        get_imsi_t() -> string(IMSI)
        
        reads IMSI_T value at address [0x6F, 0x23]
        returns IMSI_T string on success or None on error
        '''
        # select IMSI file
        imsi = self.select([0x6F, 0x23])
        if imsi is None: 
            return None
        # and parse the received data into the IMSI structure
        if 'Data' in imsi.keys() and len(imsi['Data']) == 9:
